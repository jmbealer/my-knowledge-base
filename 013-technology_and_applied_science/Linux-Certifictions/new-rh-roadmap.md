## OpenShift

    Optional:

    EX180: Red Hat Certified Technologist in OpenShift exam
    EX156: Red Hat Certified Technologist in OpenShift Virtualization exam

    Required:

    EX280: Red Hat Certified System Administrator in OpenShift exam - Systems Administrator (Level 2)
    EX380: Red Hat Certified Advanced System Administrator in OpenShift exam - Engineer (Level 3)

    Take and pass one for Specialist (Level 4) and three for Architect (Level 5): 

    EX316: Red Hat Certified Specialist in OpenShift Virtualization exam
    EX336: Red Hat Certified Specialist in Automating OpenShift Virtual Machine Management exam
    EX370: Red Hat Certified Specialist in OpenShift Data Foundation exam
    EX430: Red Hat Certified Specialist in OpenShift Advanced Cluster Security exam
    EX432: Red Hat Certified Specialist in OpenShift Advanced Cluster Management exam
    EX229: Red Hat Certified Specialist in ROSA exam 
    EX480: Red Hat Certified Specialist in MultiCluster Management exam

## Enterprise Linux

    Required:

    EX200: Red Hat Certified System Administrator (RHCSA) exam - Systems Administrator (Level 2)
    EX342: Red Hat Certified Advanced System Administrator in Enterprise Linux exam - Engineer (Level 3)

    Take and pass one for Specialist (Level 4) and three for Architect (Level 5):

    EX210: Red Hat Certified Specialist in Cloud Infrastructure exam
    EX260: Red Hat Certified Specialist in Ceph Cloud Storage exam
    EX358: Red Hat Certified Specialist in Services Management and Automation exam
    EX362: Red Hat Certified Specialist in Identity Management exam
    EX403: Red Hat Certified Specialist in Deployment and Systems Management exam
    EX415: Red Hat Certified Specialist in Security: Linux exam
    EX436: Red Hat Certified Specialist in High Availability Clustering exam
    EX442: Red Hat Certified Specialist in Performance Tuning exam

## Ansible

    Required:

    EX200: Red Hat Certified System Administrator (RHCSA) exam - Systems Administrator (Level 2)
    EX294: Red Hat Certified Advanced System Administrator in Ansible exam - Engineer (Level 3)

    Take and pass one for Specialist (Level 4) and three for Architect (Level 5):

    EX374: Red Hat Certified Specialist in Developing Automation with Ansible Automation Platform exam
    EX417: Red Hat Certified Specialist in Microsoft Windows Automation with Ansible exam
    EX457: Red Hat Certified Specialist in Ansible Network Automation exam
    EX467: Red Hat Certified Specialist in Managing Automation with Ansible Automation Platform exam

## Cloud-native applications

    Required:

    EX188: Red Hat Certified Developer in Cloud-native Applications exam - Developer (Level 2)
    EX288: Red Hat Certified Advanced Developer in Cloud-native Applications exam - Engineer (Level 3)

    Take and pass one for Specialist (Level 4) and three for Architect (Level 5):

    EX183: Red Hat Certified Specialist in Enterprise Application Development exam
    EX221: Red Hat Certified Specialist in Cloud-native Integration exam 
    EX240: Red Hat Certified Specialist in API Management exam 
    EX248: Red Hat Certified Specialist in Enterprise Application Server Administration exam
    EX328: Red Hat Certified Specialist in Building Resilient Microservices exam 
    EX378: Red Hat Certified Specialist in Cloud-native Development exam
    EX482: Red Hat Certified Specialist in Event-Driven Application Development exam

## AI

    Required:
    EX267: Red Hat Certified Developer in AI - Developer (Level 2)
