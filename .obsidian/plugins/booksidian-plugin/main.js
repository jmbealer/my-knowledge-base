...
var buildReplacementPattern = (replacements2) => {
  const sortedKeys = [...replacements2.keys()].sort((a, b) => b.length - a.length);
  return new RegExp(sortedKeys.map((key) => escapeRegex(key)).join("|"), "gu");
};
var defaultPattern = buildReplacementPattern(replacements_default);
var localeCache = new Map();
for (const [locale, localeMap] of Object.entries(locale_replacements_default)) {
  const replacements2 = new Map(replacements_default);
  for (const [key, value] of localeMap) {
    replacements2.set(key, value);
  }
  localeCache.set(locale, {
    replacements: replacements2,
    pattern: buildReplacementPattern(replacements2)
  });
}
var normalizeLocale = (locale) => {
  ...
  if (Object.hasOwn(locale_replacements_default, normalizedLocale)) {
    return normalizedLocale;
  }
  const prefix = normalizedLocale.split("-")[0];
  if (Object.hasOwn(locale_replacements_default, prefix)) {
    return prefix;
  }
};
function transliterate(string, options) {
  ...
  if (normalizedLocale) {
    ({ replacements: replacements2, pattern } = localeCache.get(normalizedLocale));
  }
  ...
  string = string.replace(pattern, (match) => {
    var _a;
    return (_a = replacements2.get(match)) != null ? _a : match;
  });
    ...
    Object.keys(this.currentYAML).forEach((key) => {
      ...
      if (key === "shelves") {
        ...
        if (Array.isArray(output[tagsOrPropertyKey])) {
          output[tagsOrPropertyKey] = output[tagsOrPropertyKey].concat(values);
        } else {
        ...
        if (value == `[[${key}]]` && this.book[key] == "") {
          ...
          const tagsOrPropertyKey = prefix === "#" ? "tags" : key;
          const value2 = this.book[key].toString();
          const sanitisedValue = prefix === "#" ? slugify(value2) : value2;
          if (Array.isArray(output[tagsOrPropertyKey])) {
            output[tagsOrPropertyKey].push(sanitisedValue.toString());
          } else if (prefix === "#") {
  ...
  createFile(book, path) {
    return __async(this, null, function* () {
      ...
      if (file && this.plugin.settings.overwritePreserveBody) {
        const existing = yield this.plugin.app.vault.read(file);
        const newFrontmatter = this.getFrontMatter(this.plugin.settings.frontmatterDictionary);
        writeFile2(fullPath, newFrontmatter + stripFrontmatter(existing), this.plugin.app);
        return;
      }
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName("Overwrite").setDesc("When syncing with Goodreads, overwrite existing notes. Modifications to notes will be lost, but changes from Goodreads will now be picked up.").addToggle((toggle) => {
      ...
      toggle.onChange((newValue) => {
        ...
        this.plugin.saveSettings();
        preserveBodySetting.settingEl.toggleClass("is-hidden", !newValue);
      });
    });
    preserveBodySetting = new import_obsidian4.Setting(containerEl).setName("Preserve note body").setDesc("When overwriting, only update the YAML frontmatter and keep the note body untouched. Useful for preserving manual edits while still picking up dynamic Goodreads fields like dateRead and shelves.").addToggle((toggle) => {
    ...
    preserveBodySetting.settingEl.addClass("booksidian-plugin__sub-setting");
    preserveBodySetting.settingEl.toggleClass("is-hidden", !this.plugin.settings.overwrite);
    ...
    Object.keys(this.currentYAML).forEach((key) => {
      ...
      new import_obsidian4.Setting(containerEl).setName(key + ": " + value).addExtraButton((button) => button.setTooltip("Convert to link").onClick(() => __async(this, null, function* () {
        ...
        yield this.plugin.saveSettings();
        this.display();
      })).setIcon("bracket-glyph").setTooltip).addExtraButton((button) => button.setTooltip("Convert to tag").onClick(() => __async(this, null, function* () {
        if (value.startsWith("[[")) {
          this.currentYAML[key] = "#" + value.replace(/[[\]]/g, "");
        } else if (value.startsWith("#")) {
          this.currentYAML[key] = value.replace(/#/g, "");
        ...
        yield this.plugin.saveSettings();
        this.display();
      })).setIcon("hash").setTooltip).addText((text) => text.setPlaceholder("").setValue(this.currentYAML[key]).onChange((value2) => __async(this, null, function* () {
